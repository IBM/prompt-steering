from prompt_steerability.persona.benchmark.utils.profiling_utils import (format_prompt)
...
class Profiler:
    def __init__(self,
                 config):
        super().__init__()
        self.profiler_config = config['profiling']
        self.profiling_method = self.profiler_config['method']
        self.inference_method = self.profiler_config['inference']['method']

        # profiling method
        if self.profiling_method == "query-principles":
            self.profile_representation = self.profiler_config['params']['representation']
            self.criteria = {
            ...
            raise ValueError(f"Unknown profiling method: {self.profiling_method}")
    ...
    def get_responses(self,
        ...
        if self.profiling_method == "query-principles":
            ...
            } for descriptor, answer in zip(descriptors, answers or [])]
